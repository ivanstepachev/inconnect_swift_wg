...
final class CustomIndicator: UIView {
    ...
    convenience init(squareLength: CGFloat) {
        ...
        let rect = CGRect(origin: CGPoint(x: (mainBounds.width-squareLength)/2, y: (mainBounds.height-squareLength)/2), size: CGSize(width: squareLength, height: squareLength))
    ...
    func startAnimation(delay: TimeInterval, replicates: Int) {
        ...
        shapeLayer.fillColor = Theme.currentTheme.indicatorColor
