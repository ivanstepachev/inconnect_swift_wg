...
class TunnelsManager {
    ...
    var closureDidGetStatus: ((NEVPNStatus) -> ())?
    static let shared = TunnelsManager()


    init(tunnelProviders: [NETunnelProviderManager]? = nil) {
        if let tunnelProviders {
    ...
    static func checkVPNConfigurationInstalled(completion: @escaping((Bool, NEVPNStatus) -> ())) {
        NETunnelProviderManager.loadAllFromPreferences { managers, error in
            if let _ = error {
                completion(false, .invalid)
            } else {

                switch managers?.first?.connection.status ?? .invalid {
                case .connected:
                    TunnelsManager.shared.tunnels = (managers ?? []).map { TunnelContainer(tunnel: $0) }.sorted { TunnelsManager.tunnelNameIsLessThan($0.name, $1.name) }
                default:
                    TunnelsManager.shared.tunnels = (managers ?? []).map { TunnelContainer(tunnel: $0) }.sorted { TunnelsManager.tunnelNameIsLessThan($0.name, $1.name) }
                }

                TunnelsManager.shared.startObservingTunnelStatuses()
                TunnelsManager.shared.startObservingTunnelConfigurations()
                completion(managers?.first?.protocolConfiguration != nil, managers?.first?.connection.status ?? .disconnected)
            }
        }
    }

    static func checkVPNStatus(completion: @escaping((NEVPNStatus) -> ())) {
        NETunnelProviderManager.loadAllFromPreferences { managers, error in
            if let _ = error {
                completion(.invalid)
            } else {
                ...
                TunnelsManager.shared.startObservingTunnelStatuses()
                TunnelsManager.shared.startObservingTunnelConfigurations()
                completion(managers?.first?.connection.status ?? .disconnected)
            }
        }
    }
...
//                        AppSettings.VPNProfileSaved = error == nil
                    }
                }
                #if os(iOS)
    ...
    func loadPreferencesIfAlreadyConnected(completionHandler: @escaping (Result<TunnelsManager, TunnelsManagerError>) -> Void) {
        NETunnelProviderManager.loadAllFromPreferences { managers, error in
            if let error = error {
                wg_log(.error, message: "Failed to load tunnel provider managers: \(error)")
                completionHandler(.failure(TunnelsManagerError.systemErrorOnListingTunnels(systemError: error)))
    ...
    func add(tunnelConfiguration: TunnelConfiguration, onDemandOption: ActivateOnDemandOption = .off, completionHandler: @escaping (Result<TunnelContainer, TunnelsManagerError>) -> Void) {
        ...
        tunnelProviderManager.localizedDescription
    ...
    func addMultiple(tunnelConfigurations: ArraySlice<TunnelConfiguration>, numberSuccessful:UInt, lastError: TunnelsManagerError?, completionHandler: @escaping (UInt, TunnelsManagerError?) -> Void) {
    ...
    func setOnDemandEnabled(_ isOnDemandEnabled: Bool, on tunnel: TunnelContainer, completionHandler: @escaping (TunnelsManagerError?) -> Void) {
        ...
        if isOnDemandEnabled {
            ...
            tunnelProviderManager.onDemandRules = [connectRule, disconnectRule]
        }
    ...
    func startActivation(of tunnel: TunnelContainer) {
        ...
        #else
        wg_log(.info, message: "Tunnel '\(tunnel.name)' startActivation called")
    ...
    func startObservingTunnelStatuses() {
        ...
        statusObservationToken = NotificationCenter.default.observe(name: .NEVPNStatusDidChange, object: nil, queue: OperationQueue.main) { [weak self] statusChangeNotification in
            guard let self = self,
                ...
                let tuP = session.manager as? NETunnelProviderManager,
                let tunnel = self.tunnels.first(where: { $0.tunnelProvider == tuP }) else { return }

            wg_log(.debug, message: "Tunnel '\(tunnel.name)' connection status changed to '\(tunnel.tunnelProvider.connection.status)'")

            self.tunnelsListDelegate?.tunnelStatusDidChange(status: tunnel.tunnelProvider.connection.status)
            ...
            if tunnel.isAttemptingActivation {
                if session.status == .connected {
                    ...
                    tunnel.isAttemptingActivation = false
                    if let (tit, msg) = lastErrorTextFromNetworkExtension(for: tunnel) {

                        self.activationDelegate?.tunnelActivationFailed(tunnel:tunnel, error:.activationFailedWithExtensionError(title: tit,message:msg, wasOnDemandEnabled:tuP.isOnDemandEnabled))
                    } else {
                        self.activationDelegate?.tunnelActivationFailed(tunnel: tunnel, error: .activationFailed(wasOnDemandEnabled: tuP.isOnDemandEnabled))
                    }
                }
            }
...
//            AppSettings.LastVpnStatus = tunnel.tunnelProvider.connection.status.rawValue
            self.closureDidGetStatus?(tunnel.tunnelProvider.connection.status)
    ...
    func startObservingTunnelConfigurations() {
        configurationsObservationToken = NotificationCenter.default.observe(name: .NEVPNConfigurationChange, object: nil, queue: OperationQueue.main) { [weak self] aa in
    ...
    func addConfiguration(tunnelsManager: TunnelsManager, fileName: String, privateKey: String, address: String, dns: String, publicKey: String, endpoint: String, allowedIPs: String, persistentKeepAlive: String) {
        let confString = "[Interface]\nPrivateKey = \(privateKey)\nAddress = \(address)\nDNS = \(dns)\n\n[Peer]\nPublicKey = \(publicKey)\nEndpoint = \(endpoint)\nAllowedIPs = \(allowedIPs)\nPersistentKeepalive = \(persistentKeepAlive)\n"

        do {
            let tunnelConfig = try TunnelConfiguration(fromWgQuickConfig: confString, called: fileName)
            print(tunnelConfig.asWgQuickConfig())

            tunnelsManager.add(tunnelConfiguration: tunnelConfig, completionHandler: { result in
                switch result {
                case .success(let container):
                    print("Tunnel Configuration Added")
                case .failure(let error):
                    self.tunnelsListDelegate?.tunnelConfigurationError(error: error.alertText.title)
                    print("Tunnel Configuration Error: \(error.alertText)")
                }
            })
        } catch {
            print(error)
    ...
    func changeConfiguration(tunnel: TunnelContainer, tunnelsManager: TunnelsManager, fileName: String, privateKey: String, address: String, dns: String, publicKey: String, endpoint: String, allowedIPs: String, persistentKeepAlive: String) {
        let confString = "[Interface]\nPrivateKey = \(privateKey)\nAddress = \(address)\nDNS = \(dns)\n\n[Peer]\nPublicKey = \(publicKey)\nEndpoint = \(endpoint)\nAllowedIPs = \(allowedIPs)\nPersistentKeepalive = \(persistentKeepAlive)\n"
        ...
        do {
            ...
            tunnelsManager.modify(tunnel: tunnel, tunnelConfiguration: tunnelConfig, onDemandOption: .off, completionHandler: { error in
                print("Tunnel Configuration modify error: \(error))")
            })
        } catch {
            print(error)
        }
...
class TunnelContainer: NSObject {
    ...
    func getRuntimeTunnelConfiguration(completionHandler: @escaping ((TunnelConfiguration?) -> Void)) {
        ...
        guard nil != (try? session.sendProviderMessage(Data([ UInt8(0) ]), responseHandler: {
            ...
            completionHandler(try? TunnelConfiguration(fromWgQuickConfig: settings))
//            completionHandler((try? TunnelConfiguration(fromUapiConfig: settings, basedOn: base)) ?? self.tunnelConfiguration)
    ...
    fileprivate func startActivation(recursionCount: UInt = 0, lastError: Error? = nil, activationDelegate: TunnelsManagerActivationDelegate?) {
        ...
        guard tunnelProvider.isEnabled else {
            ...
            let disconnectRule = NEOnDemandRuleDisconnect()
            disconnectRule.interfaceTypeMatch = .any
            tunnelProvider.onDemandRules = [connectRule, disconnectRule]
        ...
        do {
            ...
            tunnelProvider.loadFromPreferences { [weak self] error in
                guard let self = self else { return }
                if error != nil {
//                    AppSettings.VPNProfileSaved = false
...
//                AppSettings.VPNProfileSaved = true
