...
extension IPAddressRange {
    ...
    public func subnetMask() -> IPAddress {
        ...
        if address is IPv6Address {
            ...
            for ii in 0..<Int(networkPrefixLength/8) {
                bytes[ii] = 0xff
            }
            let nibble = networkPrefixLength % 32
            if nibble != 0 {
                let mask = ~UInt32(0) << (32 - nibble)
                let ii = Int(networkPrefixLength / 32 * 4)
                bytes[ii + 0] = UInt8(truncatingIfNeeded: mask >> 24)
                bytes[ii + 1] = UInt8(truncatingIfNeeded: mask >> 16)
                bytes[ii + 2] = UInt8(truncatingIfNeeded: mask >> 8)
                bytes[ii + 3] = UInt8(truncatingIfNeeded: mask >> 0)
            }
