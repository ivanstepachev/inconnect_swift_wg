...
class TunnelViewModel {
    ...
    enum InterfaceField: CaseIterable {
        ...
        var localizedUIString: String {
            ...
            case .generateKeyPair: return "Generate keypair"
            case .addresses: return "Addresses"
            case .listenPort: return "Listen port"
            case .mtu: return "MTU"
    ...
    enum PeerField: CaseIterable {
        ...
        var localizedUIString: String {
            switch self {
            case .publicKey: return "Public key"
            case .preSharedKey: return "Preshared key"
            case .endpoint: return "Endpoint"
            case .persistentKeepAlive: return "Persistent keepalive"
            case .allowedIPs: return "Allowed IPs"
            case .rxBytes: return "Data received"
            case .txBytes: return "Data sent"
            case .lastHandshakeTime: return "Latest handshake"
            case .excludePrivateIPs: return "Exclude private IPs"
            case .deletePeer: return "Delete peer"
            }
        }
    }
    ...
    class PeerData {
        ...
        func updateExcludePrivateIPsFieldState() {
            ...
            let ips = Set<String>(scratchpad[.allowedIPs].splitToArray(trimmingCharacters: .whitespacesAndNewlines))
            (shouldAllowExcludePrivateIPsControl, excludePrivateIPsValue) = TunnelViewModel.PeerData.excludePrivateIPsFieldStates(isSinglePeer: numberOfPeers == 1, allowedIPs: ips)
            shouldStronglyRecommendDNS = ips.contains(TunnelViewModel.PeerData.ipv4DefaultRouteString) || ips.isSuperset(of: TunnelViewModel.PeerData.ipv4DefaultRouteModRFC1918String)
        }
        ...
        func excludePrivateIPsValueChanged(isOn: Bool, dnsServers: String, oldDNSServers: String? = nil) {
            let allowedIPStrs = scratchpad[.allowedIPs].splitToArray(trimmingCharacters: .whitespacesAndNewlines)
            let dnsServerStrs = dnsServers.splitToArray(trimmingCharacters: .whitespacesAndNewlines)
            let oldDNSServerStrs = oldDNSServers?.splitToArray(trimmingCharacters: .whitespacesAndNewlines)
            let modifiedAllowedIPStrings = TunnelViewModel.PeerData.modifiedAllowedIPs(currentAllowedIPs:allowedIPStrs,excludePrivateIPs:isOn,dnsServers: dnsServerStrs,oldDNSServers: oldDNSServerStrs)
...
private func prettyTimeAgo(timestamp: Date) -> String {
    ...
    switch timeInterval {
    case ..<0: return "(System clock wound backwards)"
    case 0: return "Now"
    default:
        return "tunnelHandshakeTimestampAgo \(prettyTime(secondsLeft: timeInterval)) = \(prettyTime(secondsLeft: timeInterval)) ago"
    }
}
...
private func prettyTime(secondsLeft: Int64) -> String {
    ...
    #if os(iOS)
    if years > 0 {
        return years == 1 ? "tunnelHandshakeTimestampYear \(years) = \(years)" : "tunnelHandshakeTimestampYear \(years) = \(years)"
    }
    if days > 0 {
